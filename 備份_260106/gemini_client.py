...
def build_instructions() -> str:
    ...
    return (
        ...
        "- 如果日期數字在圖像中呈旋轉狀態（例如往左90度倒置），請仔細識別並正確解析。\n"
        "- 旋轉的數字可能看起來像其他數字，需要根據上下文和邏輯判斷（例如月份有31天的限制）。\n"
        ...
        "- 加班時間(起)/加班時間(迄)：這是必填欄位！如果文件中有加班記錄，必須填入加班時間區間（24小時制 HH:MM，例如 18:00、20:30）。即使原文沒有明確寫出時間，也要根據上下文推斷並填入。\n"
        "- 加班時數(分鐘)：若原文給了加班時數，填入此欄位（以分鐘為單位，純數字，如「56」，不要使用「00:56」這種時間格式）。若沒有給，可留空由系統自動計算。\n"
